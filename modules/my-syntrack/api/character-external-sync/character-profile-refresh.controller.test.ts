...
function fakeRequest(authorization: string | undefined, characterId = "char-1") {
  return {
    ...
    params: { characterId }
  } as unknown as Parameters<CharacterProfileRefreshController["refreshOne"]>[0];
}
...
function fakeResponse() {
  ...
  return {
    response: { json } as unknown as Parameters<
      CharacterProfileRefreshController["refreshOne"]
    >[1],
    json
  };
}
...
describe("CharacterProfileRefreshController auth boundary", () => {
  it("rejects refreshOne with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterProfileRefreshController(
      { refreshCharacter } as unknown as CharacterProfileRefreshService,
  ...
  it("rejects refreshAll (bulk) with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterProfileRefreshController(
      { refreshAllEligible } as unknown as CharacterProfileRefreshService,
      { requireSession } as RaiderSessionGuard
    );
