...
function fakeRequest(authorization: string | undefined, characterId = "char-1") {
  return {
    ...
    params: { characterId }
  } as unknown as Parameters<CharacterProfessionsRefreshController["refreshOne"]>[0];
}
...
function fakeResponse() {
  ...
  return {
    response: { json } as unknown as Parameters<
      CharacterProfessionsRefreshController["refreshOne"]
    >[1],
    json
  };
}
...
describe("CharacterProfessionsRefreshController auth boundary", () => {
  it("rejects refreshOne with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterProfessionsRefreshController(
      { refreshCharacter } as unknown as CharacterProfessionRefreshService,
  ...
  it("proceeds to the service once a valid session is presented", async () => {
    const refreshCharacter = vi
      .fn()
      .mockResolvedValue({ status: "SUCCESS", characterId: "char-1", professionCount: 2 });
  ...
  it("rejects refreshAll (bulk) with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterProfessionsRefreshController(
      { refreshAllEligible } as unknown as CharacterProfessionRefreshService,
      { requireSession } as RaiderSessionGuard
    );
