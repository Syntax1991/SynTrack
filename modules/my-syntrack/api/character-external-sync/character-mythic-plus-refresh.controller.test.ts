...
function fakeRequest(authorization: string | undefined, characterId = "char-1") {
  return {
    ...
    params: { characterId }
  } as unknown as Parameters<CharacterMythicPlusRefreshController["refreshOne"]>[0];
}
...
function fakeResponse() {
  ...
  return {
    response: { json } as unknown as Parameters<
      CharacterMythicPlusRefreshController["refreshOne"]
    >[1],
    json
  };
}
...
describe("CharacterMythicPlusRefreshController auth boundary", () => {
  it("rejects refreshOne with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterMythicPlusRefreshController(
      { refreshCharacter } as unknown as CharacterMythicPlusRefreshService,
  ...
  it("rejects refreshAll (bulk) with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterMythicPlusRefreshController(
      { refreshAllEligible } as unknown as CharacterMythicPlusRefreshService,
      { requireSession } as RaiderSessionGuard
    );
