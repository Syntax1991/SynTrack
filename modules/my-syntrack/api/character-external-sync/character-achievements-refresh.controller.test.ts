...
import type { CharacterAchievementsRefreshService } from "./character-achievements-refresh.service.js";
...
function fakeRequest(authorization: string | undefined, characterId = "char-1") {
  return {
    ...
    params: { characterId }
  } as unknown as Parameters<CharacterAchievementsRefreshController["refreshOne"]>[0];
}
...
function fakeResponse() {
  ...
  return {
    response: { json } as unknown as Parameters<
      CharacterAchievementsRefreshController["refreshOne"]
    >[1],
    json
  };
}
...
describe("CharacterAchievementsRefreshController auth boundary", () => {
  it("rejects refreshOne with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterAchievementsRefreshController(
      { refreshCharacter } as unknown as CharacterAchievementsRefreshService,
  ...
  it("proceeds to the service once a valid session is presented", async () => {
    const refreshCharacter = vi.fn().mockResolvedValue({
      status: "SUCCESS",
      characterId: "char-1",
      watchedAchievementCount: 5
    });
  ...
  it("rejects refreshAll (bulk) with no Authorization header before ever calling the service", async () => {
    ...
    const controller = new CharacterAchievementsRefreshController(
      { refreshAllEligible } as unknown as CharacterAchievementsRefreshService,
      { requireSession } as RaiderSessionGuard
    );
